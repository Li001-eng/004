...
# ==================== 页面配置 ====================
st.set_page_config(page_title="无人机地面站系统 - 完整监控版", layout="wide")
...
def main():
    ...
    if st.sidebar.button("🔄 刷新数据", use_container_width=True, key="refresh_data"):
    ...
    if page == "🗺️ 航线规划":
        ...
        with col1:
            ...
            if st.button("📍 设置 A 点", key="set_A", use_container_width=True):
            ...
            if st.button("📍 设置 B 点", key="set_B", use_container_width=True):
            ...
            if st.button("➕ 添加障碍物（从当前圈选）", key="add_obstacle", use_container_width=True):
            ...
            if st.button("🔄 重新规划路径（应用当前策略）", key="replan", use_container_width=True):
            ...
            with col_btn1:
                if st.button("▶️ 开始飞行", key="start_flight", use_container_width=True):
            ...
            with col_btn2:
                if st.button("⏹️ 停止飞行", key="stop_flight", use_container_width=True):
                    st.session_state.simulation_running = False
        ...
        with col2:
            ...
            output = st_folium(m, width=700, height=550, key="planning_map", returned_objects=["last_active_drawing"])
    ...
    # ==================== 飞行监控页面（增强版） ====================
        ...
        with col_btn1:
            if st.button("▶️ 开始任务", key="start_mission", use_container_width=True):
                if not st.session_state.simulation_running:
                    ...
                    st.rerun()
        with col_btn2:
            if st.button("⏸️ 暂停", key="pause_mission", use_container_width=True):
                if st.session_state.simulation_running:
                    st.session_state.heartbeat_sim.pause()
                    st.rerun()
        with col_btn3:
            if st.button("⏹️ 停止", key="stop_mission", use_container_width=True):
                st.session_state.simulation_running = False
                st.session_state.heartbeat_sim.stop()
                st.rerun()
        ...
        if st.session_state.heartbeat_sim.history:
            ...
            with col3:
                remaining_dist_m = max(0, (latest.get('total_distance',0)-latest.get('distance_traveled',0))*111000)
                st.metric("剩余距离", f"{remaining_dist_m:.0f} m")
            with col4:
            ...
            col1, col2, col3, col4, col5 = st.columns(5)
            col1.metric("当前航点", "0/0")
            col2.metric("飞行速度", "0 m/s")
            col3.metric("剩余距离", "0 m")
            col4.metric("预计到达", "00:00")
            col5.metric("电量模拟", "0%")
...
if col_save.button("💾 保存到缓存", use_container_width=True, key="save_cache"):
    save_obstacles_to_cache()
if col_load.button("📂 从缓存加载", use_container_width=True, key="load_cache"):
...
if col_clear_all.button("🗑️ 全部清除", use_container_width=True, key="clear_all"):
    st.session_state.obstacles_gcj = []
    st.session_state.planned_path = create_avoidance_path(
        st.session_state.points_gcj['A'],
        st.session_state.points_gcj['B'],
