...
# ==================== 页面配置 ====================
st.set_page_config(page_title="无人机地面站系统 - 飞越/绕行双模式", layout="wide")
...
def rrt_star_planning(start, end, obstacles_gcj, safe_radius_m, flight_height,
                       max_iter=2000, step_size=0.0002, goal_sample_rate=0.2, search_radius=0.0005):
    """
    RRT* 渐进最优路径规划算法
        ...
        if len(path) > 2:
            ...
            while current_idx < len(path) - 1:
                ...
                while next_idx > current_idx:
                    if is_path_safe(path[current_idx], path[next_idx], obstacles_gcj, safe_radius_m, flight_height):
                        break
                    next_idx -= 1
...
def create_avoidance_path(start, end, obstacles_gcj, flight_height, safe_radius_m, strategy):
    ...
    if is_path_safe(start, end, obstacles_gcj, safe_radius_m, flight_height):
        return [start, end]
    # 直线不安全，使用RRT*算法绕行
    return rrt_star_planning(start, end, obstacles_gcj, safe_radius_m, flight_height)
...
def main():
    ...
    strategy = st.sidebar.selectbox("选择避障方式", ["RRT* 最优绕行", "A* 最佳航线", "向左绕行", "向右绕行"], index=0)
    ...
    if page == "🗺️ 航线规划":
        ...
        if not straight_safe:
            st.warning(f"⚠️ 直线航线被需绕行的障碍物阻挡！已自动规划RRT*最优避障航线（绿色）")
        else:
        ...
        with col1:
            ...
            if st.button("🔄 重新规划路径", use_container_width=True):
                with st.spinner("正在规划RRT*最优绕行路径..."):
                    st.session_state.planned_path = create_avoidance_path(
                        st.session_state.points_gcj['A'],
                        st.session_state.points_gcj['B'],
                        st.session_state.obstacles_gcj,
    ...
    # ==================== 障碍物管理页面 ====================
        ...
        with col1:
            if st.session_state.obstacles_gcj:
                for i, obs in enumerate(st.session_state.obstacles_gcj):
                    ...
                    col_height.write(f"{obs.get('height',20)}m {'⚠️' if need_avoid else '✅'}")
            ...
            if col_load.button("📂 从缓存加载", use_container_width=True):
                if load_obstacles_from_cache():
                    ...
                    st.rerun()
            col_clear_all, col_refresh = st.columns(2)
        ...
        with col2:
            ...
            add_safety_buffer(obs_map, st.session_state.obstacles_gcj, safe_radius, st.session_state.flight_altitude)
            for obs in st.session_state.obstacles_gcj:
                coords = obs.get('polygon', [])
                if coords and len(coords) >= 3:
                    need_avoid = should_avoid_obstacle(obs, st.session_state.flight_altitude)
            ...
            folium_static(obs_map, width=800, height=600)
