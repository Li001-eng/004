...
# ==================== 页面配置 ====================
st.set_page_config(page_title="无人机地面站系统 - 完整功能版", layout="wide")
...
class HeartbeatSimulator:
    ...
    def _generate_heartbeat(self):
        altitude = self.flight_altitude + random.randint(-5,5) if self.simulating else 0
...
def main():
    ...
    strategy = st.sidebar.radio("选择避障方式", ["最佳航线", "向左绕行", "向右绕行"], index=0)
    strategy_map = {"最佳航线": "best", "向左绕行": "left", "向右绕行": "right"}
    ...
    if page == "🗺️ 航线规划":
        st.header("🗺️ 航线规划")
        if straight_blocked:
            st.warning(f"⚠️ 直线航线被阻挡！已规划 {strategy} 避障航线")
        else:
            st.success(f"✅ 直线航线安全")
        st.info("📝 点击地图左上角📐图标 → 绘制多边形 → 双击完成 → 侧边栏设置高度 → 点击「添加障碍物」")
        ...
        with col2:
            ...
            st.caption("📌 图例：绿=避障航线，红=障碍物，橙=安全缓冲区")
    ...
    # ==================== 飞行监控页面 ====================
        ...
        if st.session_state.heartbeat_sim.history:
            ...
            col1.metric("当前航点", latest.get('current_waypoint', '0/0'))
            col2.metric("飞行速度", f"{latest.get('speed', 0)} m/s")
            col3.metric("剩余距离", f"{max(0, (latest.get('total_distance',0)-latest.get('distance_traveled',0))*111000):.0f} m")
            col4.metric("预计到达", latest.get('remaining_time', '00:00'))
            col5.metric("电量模拟", f"{latest.get('battery', 0)}%")

            progress_val = latest.get('progress', 0)
        ...
        if st.session_state.heartbeat_sim.history:
            ...
            if st.session_state.planned_path and len(st.session_state.planned_path) > 1:
                folium.PolyLine([[p[1], p[0]] for p in st.session_state.planned_path], color="green", weight=3, opacity=0.7).add_to(monitor_map)
            ...
            if st.session_state.points_gcj['A']:
                folium.Marker([st.session_state.points_gcj['A'][1], st.session_state.points_gcj['A'][0]],
                              popup="起点", icon=folium.Icon(color='green')).add_to(monitor_map)
            if st.session_state.points_gcj['B']:
                folium.Marker([st.session_state.points_gcj['B'][1], st.session_state.points_gcj['B'][0]],
                              popup="终点", icon=folium.Icon(color='red')).add_to(monitor_map)
    ...
    # ==================== 障碍物管理页面 ====================
        ...
        st.header("🚧 障碍物管理")
        st.info(f"当前共 **{len(st.session_state.obstacles_gcj)}** 个障碍物（含高度）")
        col1, col2 = st.columns([1, 1.5])
        with col1:
        ...
        with col2:
            ...
            folium.Marker([DEFAULT_A_GCJ[1], DEFAULT_A_GCJ[0]], popup="起点", icon=folium.Icon(color='green')).add_to(obs_map)
            folium.Marker([DEFAULT_B_GCJ[1], DEFAULT_B_GCJ[0]], popup="终点", icon=folium.Icon(color='red')).add_to(obs_map)
