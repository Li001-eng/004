...
def line_cross_poly(p1, p2, poly):
    ...
    for i in range(len(poly)):
        if lines_intersect(p1, p2, poly[i], poly[(i+1)%len(poly)]):
            return True
    return False
...
def main():
    ...
    if 'hb' not in st.session_state:
        st.session_state.hb = HeartbeatManual(st.session_state.waypoints[0][:])
    ...
    if page == "规划":
        ...
        with col2:
            ...
            m = make_map(center, st.session_state.waypoints, st.session_state.obs, st.session_state.hist,
                         st.session_state.full_path, map_type, st.session_state.safe_rad, st.session_state.alt)
