...
# 坐标转换函数（完整实现）
def gcj2wgs(lng, lat):
    if abs(lng) < 72 or abs(lng) > 138 or abs(lat) < 0.8 or abs(lat) > 56:
        return lng, lat
    ...
    dlng = (dlng * 180.0) / (6378245.0 / sqrtmagic * math.cos(rad) * math.pi)
...
def line_cross_poly(p1, p2, poly):
    ...
    n = len(poly)
    for i in range(n):
        if lines_intersect(p1, p2, poly[i], poly[(i + 1) % n]):
            return True
    return False
...
def path_safe(p1, p2, obs, rad_m, h):
    for o in obs:
        if not should_avoid(o, h):
            continue
        poly = o.get('polygon', [])
        if len(poly) < 3:
            continue
        if line_cross_poly(p1, p2, poly):
            return False
        if seg_to_poly_dist(p1, p2, poly) < rad_m - 0.1:
            return False
    return True
...
class Heartbeat:
    ...
    def update(self):
        ...
        if self.idx < len(self.path) - 1:
            ...
            step_m = (0.5 + self.spd / 100 * 4.5) * 0.2
...
def make_map(center, waypoints, obs, hist, full_path, maptype, rad, h):
    ...
    if full_path and len(full_path) > 1:
        ...
        for p in full_path[1:-1]:
            folium.CircleMarker([p[1], p[0]], 3, color='green', fill=True).add_to(m)
    if len(waypoints) > 1:
...
def main():
    ...
    if 'hb' not in st.session_state:
        st.session_state.hb = Heartbeat(st.session_state.waypoints[0][:])
    if 'last_time' not in st.session_state:
        st.session_state.last_time = time.time()
    if 'running' not in st.session_state:
        st.session_state.running = False
    if 'alt' not in st.session_state:
        st.session_state.alt = 50
    if 'hist' not in st.session_state:
        st.session_state.hist = []
    if 'full_path' not in st.session_state:
        st.session_state.full_path = None
    if 'pending_poly' not in st.session_state:
        st.session_state.pending_poly = None
    if 'pending_h' not in st.session_state:
        st.session_state.pending_h = 20
    if 'drone_spd' not in st.session_state:
        st.session_state.drone_spd = 50
    if 'safe_rad' not in st.session_state:
        st.session_state.safe_rad = 5
    if 'sel_strat' not in st.session_state:
        st.session_state.sel_strat = 'best'
    if 'new_wp_lng' not in st.session_state:
        st.session_state.new_wp_lng = A_DFT[0]
    if 'new_wp_lat' not in st.session_state:
        st.session_state.new_wp_lat = A_DFT[1]
    ...
    if page == "规划":
            ...
            if st.button("➕ 添加障碍物"):
                if st.session_state.pending_poly and len(st.session_state.pending_poly) >= 3:
                    st.session_state.obs.append({
                        "name": f"建筑物{len(st.session_state.obs) + 1}",
        ...
        if st.session_state.hb.hist:
            ...
            d = {"current_wp": "0/0", "speed": 0, "elapsed": 0, "total": 0, "traveled": 0,
                 "remain": "00:00", "battery": 0, "progress": 0, "delay_ms": 0, "loss_percent": 0}
        # 指标显示
        row1 = st.columns(4)
        ...
        with col2:
            ...
            for o in st.session_state.obs:
                coords = o.get('polygon', [])
                if len(coords) >= 3:
                    folium.Polygon([[c[1], c[0]] for c in coords], color='red', weight=3, fill=True, fill_opacity=0.5,
                                   popup=f"高度:{o.get('height', 20)}m").add_to(m)
